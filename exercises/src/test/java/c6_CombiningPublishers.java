...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        //todo: feel free to change code as you need
        Flux<Void> tasks = taskExecutor().flatMap(f -> f);
    ...
    public void streaming_service() {
        //todo: feel free to change code as you need
        Flux<Message> messageFlux = streamingService().flatMapMany(item -> item);
...
//        Flux<Void> tasks = taskExecutor().concatMap(voidMono -> voidMono);
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((first, flux) -> {
            if (first.hasValue() && first.get().metaData.equals("spam")) {
    ...
    public void one_to_n() {
        //todo: feel free to change code as you need
        Flux<String> fileLines = Mono.when(openFile()).thenMany(readFile());
    ...
    public void acid_durability() {
        ...
        Flux<String> committedTasksIds = tasksToExecute()
                .flatMapSequential(item ->
                        item.flatMap(i -> commitTask(i).thenReturn(i)));
    ...
    public void car_factory() {
        //todo: feel free to change code as you need
        Flux<Car> producedCars = Flux.zip(carChassisProducer(), carEngineProducer()).map(t -> new Car(t.getT1(), t.getT2()));
