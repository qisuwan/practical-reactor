...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void error_reporter() {
        //todo: feel free to change code as you need
        Flux<String> messages = messageNode()
                .onErrorResume(error -> errorReportService(error).then(Mono.error(error)));
    ...
    public void billion_dollar_mistake() {
        Flux<String> content = getFilesContent()
                ...
                //todo: change this line only
                .onErrorContinue((error, task) -> System.out.println(error.getMessage()))
                ;
    ...
    public void resilience() {
        ...
        Flux<String> content = getFilesContent()
                .flatMap((t) -> t.onErrorResume(error -> Mono.empty())); //start from here
...
//                .doOnSubscribe(s -> System.out.println(LocalDateTime.now() + "on sub"))
                //todo: change this line only
    ...
    public void good_old_polling() {
        ...
        Flux<String> alerts = nodeAlerts()
                .repeatWhenEmpty(m -> m.delayElements(Duration.ofSeconds(1))).repeat();
